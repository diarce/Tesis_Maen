...
JS_RENDER_CONFIG = {
    ...
    # Rutas candidatas del Chromium del SISTEMA (fallback secundario, si
    # existiera un binario del sistema). NOTA: NO declarar el paquete apt
    # 'chromium' en packages.txt de Streamlit Cloud — su árbol de
    # dependencias puede actualizar librerías compartidas del sistema y
    # provocar fallos (segfault) del propio Streamlit. El patrón correcto:
    # packages.txt solo con las librerías livianas que el navegador
    # necesita (incluido en el proyecto) + descarga del navegador en
    # runtime ('playwright install chromium', automática en el primer uso).
    ...
    # Auto-instalación del navegador en runtime si no se encuentra ninguno
    # (patrón para Streamlit Cloud). Descarga ~130 MB al home del usuario
    # (~/.cache/ms-playwright) la primera vez; no toca el sistema.
    "auto_install_browser"        : True,
    "auto_install_timeout_seconds": 600,
}
