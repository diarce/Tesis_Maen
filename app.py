    ...
    try:
        ...
        if st.session_state.js_no_disponible:
            add_log(
                ...
                "verificar que packages.txt (con las librerías del sistema, "
                "incluido en el proyecto) esté en la raíz del repositorio; "
                "el navegador se descarga solo en el primer uso.", "ERR",
            )
        st.session_state.audit_done = True
        st.session_state.panel      = "qa"
...
def _grupo_parametros():
    ...
    st.session_state.selenium_enabled = st.toggle(
        ...
        help  = (
            ...
            "un 'No cumple' real. Local: requiere `playwright install chromium`. "
            "Streamlit Cloud: incluir packages.txt (provisto) en la raíz del "
            "repositorio; el navegador se descarga automáticamente en el "
            "primer uso (puede demorar 1-3 minutos esa única vez). Si no está "
            "disponible, la app lo detecta y continúa con el resultado "
            "estático sin interrumpir la auditoría."
        ),
    )
...
def _panel_qa():
    ...
    if st.session_state.get("js_no_disponible"):
        st.warning(
            ...
            "Local: `playwright install chromium` · Streamlit Cloud: verificar "
            "que `packages.txt` (con las librerías del sistema, incluido en el "
            "proyecto) esté en la raíz del repositorio y redesplegar; el "
            "navegador se descarga automáticamente en el primer uso."
        )
